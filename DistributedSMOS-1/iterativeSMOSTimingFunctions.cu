...
template results_t<int>* timeSortAndChooseMultiselect
	(int * h_vec, uint numElements, uint * kVals, uint kCount);
template results_t<unsigned int>* timeSortAndChooseMultiselect
	(unsigned int * h_vec, uint numElements, uint * kVals, uint kCount);
template results_t<float>* timeSortAndChooseMultiselect
	(float * h_vec, uint numElements, uint * kVals, uint kCount);
template results_t<double>* timeSortAndChooseMultiselect
	(double * h_vec, uint numElements, uint * kVals, uint kCount);
...
template<typename T>
results_t<T>* timeIterativeSMOS (T * h_vec, uint numElements, uint * kVals, uint kCount) {
  T * d_vec;
  results_t<T> * result;
  float time;
  cudaEvent_t start, stop;
  ...
  // bucketMultiselectWrapper (T * d_vector, int length, uint * kVals_ori, uint kCount, T * outputs, int blocks, int threads)
  IterativeSMOS::iterativeSMOSWrapper(d_vec, numElements, kVals, kCount, result->vals, dp.multiProcessorCount, dp.maxThreadsPerBlock);
