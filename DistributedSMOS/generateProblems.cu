...
ptrToUintGeneratingFunction arrayOfUintGenerators[NUMBEROFUINTDISTRIBUTIONS]
        = {&generateUniformUnsignedIntegers,&generateSortedArrayUints,&generateUniformZeroToFourUints};
char* namesOfUintGeneratingFunctions[NUMBEROFUINTDISTRIBUTIONS]
        = {"UNIFORM UNSIGNED INTEGERS","SORTED UINTS","UNIFORM 0-4"};
...
__global__ void createVector(float* input, int length, int firstVal, int numFirstVal,
                             int secondVal, int numSecondVal, int offset){
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
...
ptrToFloatGeneratingFunction arrayOfFloatGenerators[NUMBEROFFLOATDISTRIBUTIONS]
            = {&generateUniformFloats, &generateNormalFloats,&generateBucketKillerFloats,
...
char* namesOfFloatGeneratingFunctions[NUMBEROFFLOATDISTRIBUTIONS]
            ={"UNIFORM FLOATS","NORMAL FLOATS","KILLER FLOATS",
...
ptrToDoubleGeneratingFunction arrayOfDoubleGenerators[NUMBEROFDOUBLEDISTRIBUTIONS]
            = {&generateUniformDoubles,&generateNormalDoubles, &generateBucketKillerDoubles};
char* namesOfDoubleGeneratingFunctions[NUMBEROFDOUBLEDISTRIBUTIONS]
            = {"UNIFORM DOUBLES","NORMAL DOUBLES", "KILLER DOUBLES"};
...
template void* returnGenFunctions(int type);
template void* returnGenFunctions(unsigned int type);
template void* returnGenFunctions(float type);
template void* returnGenFunctions(double type);
